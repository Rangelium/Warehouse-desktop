  ...
  pool.request()
    .execute("dbo.products_return", (err, res) => {
      ...
      for(let result of data){
        $("#productList").append($("<option>", {value: result["product_id"], text: result["title"]}));
...
$("#completeInventory").click(() => {
  ...
  poolConnect.then((pool) => {
    pool.request()
        .execute("inventory.finish", (err, res) => {
          console.log(err, res);
          setTimeout(() => {
...
function addInput(){
  ...
  for(let i = 0; i < updateButtons.length; i++){
    updateButtons[i].onclick = function() {
      ...
      $('#productId').val(parentLi.firstElementChild.children.item(2).textContent);
      $("#itemQuantity").val(parseInt(parentLi.firstElementChild.children.item(1).textContent));
  ...
  data.forEach((element) => {
    ...
    for(let i = 0; i < updateButtons.length; i++){
      updateButtons[i].onclick = function() {
        ...
        $("#itemQuantity").val(parseInt(parentUl.firstElementChild.children.item(1).textContent));
      }
    }
  })
}
...
function fillInventoryList(data){
  ...
  data.forEach((elem) => {
    ...
    inventoryDate.innerHTML = moment(creation_date).format('YYYY/MM/DD hh:mm:ss');
