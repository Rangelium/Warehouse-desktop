...
function fillTable(){
  poolConnect.then((pool) => {
    pool.request()
        .execute("dbo.user_select_all", (err, res) => {
          data = [];
          ...
          $(".userList-table-data").append("<table class='userListProductTable'></table>");
          $(".userListProductTable").append("<thead></thead>");
          $(".userListProductTable").append("<tbody></tbody>");
      
          $(".userListProductTable > thead").append(`<th>Username:</th>`);
          $(".userListProductTable > thead").append(`<th>Name:</th>`);
          $(".userListProductTable > thead").append(`<th>Surname:</th>`);
          $(".userListProductTable > thead").append(`<th>Privelage:</th>`);
...
poolConnect.then((pool) => {
  pool.request()
      .execute("dbo.user_privelage_select", (err, res)=>{
        let data = []
        for(let i of res.recordset){
          data.push(i);
        }
        for(let result of data){
          $("#privelageSelect").append($("<option>", {value: result["id"], text: result["title"]}));
        }
      })
})
